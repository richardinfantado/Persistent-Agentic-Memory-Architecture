...
All requirement identifiers (e.g., `PAMSPEC.scope.immutable_across_update`) are the canonical `requirement_id` values appearing in `validation/evidence/mem0-r5-portability.jsonl` and `validation/evidence/mem0-v08-validation.jsonl`. All probe identifiers (e.g., `v08.s1.scope-immutability.probe`) are `record_id` values in those same files.
...
|---|---|---|---|
| `v08.s2.identity-history.probe` | `PAMSPEC.identity.stable_object_and_history_ledger` | native | `mem0-v08-validation.jsonl` |
...
| record_id | requirement_id | classification | source |
|---|---|---|---|
| `v08.s1.scope-immutability.probe` | `PAMSPEC.scope.immutable_across_update` | gap | `mem0-v08-validation.jsonl` |
...
| record_id | requirement_id | classification | source |
|---|---|---|---|
| `v08.s8.unknown-fields.probe` | `PAMSPEC.extensibility.unknown_fields_survive_roundtrip` | gap | `mem0-v08-validation.jsonl` |
...
|---|---|---|---|
| `v08.s2.identity-history.probe` | `PAMSPEC.identity.stable_object_and_history_ledger` | native | `mem0-v08-validation.jsonl` |
| `v08.s3.expected-version.probe` | `PAMSPEC.mutation.expected_version_conflict_rejection` | gap | `mem0-v08-validation.jsonl` |
...
| record_id | requirement_id | classification | source |
|---|---|---|---|
| `v08.s4.idempotency.probe` | `PAMSPEC.mutation.idempotent_create` | gap | `mem0-v08-validation.jsonl` |
...
**Status note:** CR-7 is a candidate Core requirement. The draft anchors cited below address canonical state authority and sequence monotonicity but do not yet contain a direct MUST for bundle-deterministic serialization. PAMSPEC-Lite has no test case that directly exercises bundle determinism. CR-7 is included here as a provisional requirement; it requires a direct normative draft statement and a dedicated conformance test before it can be treated as fully established Core. See OD-C3 in `R8-OPEN-DECISIONS.md`.
...
### Conformance test cases (PAMSPEC-Lite)
No direct Lite test case for bundle determinism. Covered indirectly by:
- `case_history_is_monotonic` (proxy: verifies canonical version order is stable)

A dedicated `case_bundle_output_is_deterministic` test is required before CR-7 can be promoted from provisional to established Core (OD-P1 / R9 prep).
...
| record_id | requirement_id | classification | source |
|---|---|---|---|
| `v08.s5.tombstone.probe` | `PAMSPEC.deletion.tombstone_and_identity_reservation` | gap | `mem0-v08-validation.jsonl` |
...
| record_id | requirement_id | classification | source |
|---|---|---|---|
| `v08.s6.provenance.probe` | `PAMSPEC.provenance.source_actor_and_activity` | gap | `mem0-v08-validation.jsonl` |
...
| CR | requirement_id(s) | Evidence classification | reference-python | Mem0EnforcementAdapter |
|---|---|---|---|---|
| CR-1 Stable Identity | `PAMSPEC.identity.stable_object_and_history_ledger` | native | native | native (Mem0 behavior) |
| CR-2 Immutable Scope | `PAMSPEC.scope.immutable_across_update` | Mem0 gap / adapter-enforced | native | adapter-enforced |
| CR-3 Canonical Content | `PAMSPEC.extensibility.unknown_fields_survive_roundtrip`, `PAMSPEC.portability.mem0_to_refimpl_round_trip` | Mem0 gap / adapter-emulated portability | native | adapter-enforced (sidecar) |
| CR-4 Immutable Versions | `PAMSPEC.identity.stable_object_and_history_ledger`, `PAMSPEC.mutation.expected_version_conflict_rejection` | native (history log) / Mem0 gap (version_id) | native | partially native + adapter-enforced |
| CR-5 Expected-Version Mutation | `PAMSPEC.mutation.expected_version_conflict_rejection` | Mem0 gap / adapter-enforced | native | adapter-enforced |
| CR-6 Idempotency | `PAMSPEC.mutation.idempotent_create`, `PAMSPEC.mutation.idempotent_create.durable_across_restart` | Mem0 gap / adapter-enforced | native | adapter-enforced (SQLite) |
| CR-7 *(provisional)* Deterministic Outcomes | `PAMSPEC.portability.deterministic_bundle_output` | adapter-emulated portability | emulated (bundle format) | emulated (bundle format) |

**Evidence classification key:**
- **native** — the underlying framework provides this behavior without adapter intervention
- **Mem0 gap / adapter-enforced** — Mem0 does not provide this behavior; the adapter enforces it via the SQLite sidecar
- **adapter-emulated portability** — the bundle format layer (not Mem0 or reference-python storage) provides the guarantee
- **Mem0 gap / adapter-emulated portability** — Mem0 discards this information natively; the adapter sidecar stores it and the bundle format preserves it across implementations

**CR-7 note:** provisional because no direct normative bundle-determinism MUST exists in the draft and no PAMSPEC-Lite test directly verifies it (OD-C3, OD-P1). CR-1 through CR-6 are the established proposed Core.
