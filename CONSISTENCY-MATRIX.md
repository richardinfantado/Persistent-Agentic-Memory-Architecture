...
| Idempotency | Section 12 | `idempotency_key` on create/update | `idempotent-repeated-create.json` | `duplicate-operation-different-content.json` | PAMSPEC-Core, PAMSPEC-Lite | Prevents replay-driven duplication | Key + request-digest table |
...
| `resource_usage` on events | Section 8 (Event Ledger) | `memory-event.schema.json` `resource_usage` | `event-with-resource-usage.json` | `event-with-negative-tokens.json` (C2) | PAMSPEC-Ledger | Cost data may be sensitive | Per-event token / cost accounting |
| Subscribe operation | Section 10 (Operation Semantics) | `subscription.schema.json` | subscription descriptor test | `subscription-negative-start-sequence.json` (C2) | PAMSPEC-Ledger | Per-event authorization re-eval | At-least-once delivery, event_id dedup |
| Delegation Object | Section 15 (Security Considerations) | `delegation.schema.json` | `delegation.json` | `delegation-window-inverted.json`, `delegation-unknown-operation.json` (C2) | PAMSPEC-Core | Bounds confused-deputy blast radius | Bounded time/ops/objects; `policy_basis` mandatory |
